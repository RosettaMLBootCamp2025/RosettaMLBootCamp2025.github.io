...
::: {.callout-note}
## General environment vs. model environments
This is a lightweight readiness environment, not a single environment for every ML model. Use the exact, isolated tool environments and version records described in [Reproducible Setup & Compute Fallbacks](reproducibility.qmd). After this environment passes verification, save `conda env export --from-history` and `conda list --explicit` outputs with your result.
:::
...
For Mac/Windows, download and install from: [https://www.anaconda.com/download](https://www.anaconda.com/download)
...
### Step 2: Create Your Working Directory & Download the Setup Files
...
3.  Download the two setup files below and save them **into that `HW1-setup` folder**:

<a href="files/environment.yml" class="btn btn-primary" download>
  <i class="bi bi-download"></i> Download environment.yml
</a>
<a href="files/verify_setup.py" class="btn btn-primary" download>
  <i class="bi bi-download"></i> Download verify_setup.py
</a>

::: {.callout-tip}
If your browser opens a file in a new tab instead of downloading it, right-click the button and choose **"Save link as…"**. Make sure the files keep their exact names (`environment.yml` and `verify_setup.py`).
:::
