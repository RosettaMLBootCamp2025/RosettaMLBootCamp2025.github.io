...
::: {.callout-tip}
## Save a preflight report first
From the course repository, run `python scripts/preflight.py --output preflight-report.txt` (or `python ../scripts/preflight.py` when your shell is inside `monday/`). The report is safe to generate on a login node and tells you when to use the [documented no-compute pathway](reproducibility.qmd).
:::
...
```bash
wget https://github.com/conda-forge/miniforge/releases/download/26.5.3-0/Miniforge3-Linux-x86_64.sh
echo "14db468222ad564658656f769506056209b6dc375f5e7dfd31eb5ebbf08fa529  Miniforge3-Linux-x86_64.sh" | sha256sum --check
