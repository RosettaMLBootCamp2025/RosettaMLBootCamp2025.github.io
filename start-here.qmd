...
Start with a **15-minute readiness check**. The recommended route uses hosted notebooks; choose supplied examples if you do not have compute or account access.

[Begin the readiness check ↓](#your-15-minute-readiness-check){.btn .btn-primary}

```{=html}
<section id="route-picker" aria-label="Choose your learning path"></section>
<noscript><p>Follow the <a href="monday/index.html#pre-work-and-readiness">core readiness checklist</a>. Route selection and saved progress require JavaScript.</p></noscript>
```
...
If a required answer is “not yet,” begin with [Monday pre-work](monday/index.qmd#pre-work-and-readiness). GPU access is not required for the hosted or analysis route. Your learning plan above skips local installations on those routes.

[My readiness note names my route, compute, storage, accounts, and fallback]{.task module-id="monday-readiness-evidence"}
...
You can still learn the scientific reasoning. Open the [supplied analysis datasets](analysis-lab.qmd) and follow the first task instead of running the job. In your evidence portfolio, label the source of those outputs and explain what you would change in a new run. Running a model is not the same as interpreting it well.
:::
...
## Set up your portfolio at the first hands-on task

When you reach your first hands-on activity, [download the portfolio starter](files/portfolio-starter.zip) and unzip it. It includes readiness and decision-log templates. A simple structure is enough:
